...
$edno = $_POST["edno"]; //POSTの値を受け取って変数に割り当てる

if ($pass == $_POST["pass"]) { 	//パスワードが正しければ
$data = file("mission_2-6.txt"); //ログファイルを開いて
$linecount = 0; //行数カウント
...
$deldata = fopen("mission_2-6.txt", "w"); 	//書き込みモードでファイルを開く
...
$resline = file ("mission_2-6.txt");	//データを開いて
...
$pass = $_POST["pass"]; //POSTの値を受け取って変数に割り当てる
if ($pass == $_POST["pass"]) { 	//パスワードが正しければ
$ediline = file("mission_2-6.txt");	//データを開いて
...
print "<form action='mission_2-6.php' method='POST'>\n";
...
$nip = ($host . "/" . $ip);
$data = file("mission_2-6.txt");//ファイルを開いて
$bno = 0;
...
$redata = fopen("mission_2-6.txt", "w");	//書き込みモードでファイルを開く
...
$edi = fopen('mission_2-6.txt', 'w');//書き込みモードでデータを開く
flock($edi, LOCK_EX);	//ファイルロック
foreach($edit as $value) {
fputs($edi, $value);	//１行置き換えして書き込む
}
flock($edi, LOCK_UN); 	//ロック解除
fclose($edi);	//閉じる

}
else {	//新規投稿の場合
$topwrite = "$no<>$no<>$name<>$email<>$url<>$title<>$comment<>$time<>$nip<>\n"; //追加する１行
$data = file("mission_2-6.txt");//データを読み込む
$nwrite = fopen("mission_2-6.txt","w"); //追加書き込み　空になる
...
$nwrite = fopen("mission_2-6.txt", "w");
...
$log = file("mission_2-6.txt"); //ログファイルを開く
$data = fopen("mission_2-6.txt", "r"); //データを読み込む
...
print $list[7] . "　[" . $list[0] . "]　<a href='mission_2-6.php?resformno=" . $list[0] . "'>Re:</a><br>";
print $list[6];
print "<hr>"; 	//記事の最後に線を表示
}
else { //（表示）返信記事なら
...
print "<a href='mission_2-6.php?line=" . "$next" . "'>" . "$now" . "</a>&nbsp;";}//アンカータグで GET で渡す
$now++;
$next = $next + 1;
$lines = $lines - 1;
}
...
<form action="mission_2-6.php?page=20&no=2" method="POST">
Number <input size="4" name="edno">　
PASS <input type="password" size="10" name="pass">
